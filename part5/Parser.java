...
public class Parser {
	...
	private boolean statement() {
		if (is(TK.PRINT))
			...
			assignment();
		else if( is(TK.FOR) )	//part for_fcn
			for_fcn();
		else 
			return false;
		return true;
	...
	private void for_fcn() {
		...
		//if(temp.equals(tok.string)){
			...
			mustbe(TK.SEMI);
			//if(temp.equals(tok.string)){
			System.out.print("x_" + (symtab.size()-1)+ tok.string);	//error
			mustbe(TK.ID);
                        if(!is(TK.ENDFOR)) {
			...
			/*}
				...
				System.err.println("Not the same ID");
				System.exit(1);
			}*/
